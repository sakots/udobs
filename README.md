...
OBSの「ツール」→「WebSocketサーバー設定」を開き、次を確認してください。

- 「WebSocketサーバーを有効にする」がオンであること
- `.env` の `OBS_PASSWORD` が、OBSに設定したパスワードと完全に一致すること
- `OBS_URL` のポート番号がOBS側の設定と一致すること

パスワードは必ず引用符で囲んでください。`#`、空白などが含まれていても正しく読み込めます。

```dotenv
OBS_PASSWORD="ここにOBSのパスワードをそのまま貼り付ける"
```

パスワードを変更した場合は、OBSで「適用」してからブリッジを再起動してください。起動ログに `OBS WebSocketの認証が完了しました` が出るまでは、OBSへの字幕更新は行われません。
