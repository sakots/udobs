...
export function parseConfig(argv, env = process.env) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    config[key] = ['reconnectMs', 'pollMs'].includes(key) ? Number(value) : value.trim();
    index += 1;
  }
...
同名の環境変数（OBS_INPUT_NAME, OBS_PASSWORD, OBS_URL, UDTALK_PUBLIC_URL, POLL_MS, RECONNECT_MS）も使えます。`;
